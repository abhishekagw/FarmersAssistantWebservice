...
<div class="row">
   <?php
	$i=0;
  $search=$_GET["sid"];
	$disQry="select * from tbl_request a inner join  tbl_category c on a.category_id=c.category_id inner join tbl_retailer r on a.retailer_id=r.retailer_id inner join tbl_place p on p.place_id=r.place_id  inner join tbl_district d on p.district_id=d.district_id  WHERE a.request_product LIKE '%$search%' AND a.category_id= ".$_GET['cid'];
