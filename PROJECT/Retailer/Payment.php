...
{
	$ins="update tbl_booking set booking_status=1,payment_status=1 where booking_id=".$_GET['bid'];
	$updateQry=$con->query($ins);
	?>
    <script>
...
$selQ="select * from tbl_booking b inner join tbl_product t on t.product_id=b.product_id where booking_id='".$_GET["bid"]."'";
$resBooking=$con->query($selQ);
 $bookingData=$resBooking->fetch_assoc();
 $rate=$_GET["aid"];
?>
<!DOCTYPE html>
<html lang="en">
